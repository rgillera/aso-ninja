...
const STORE_LABEL: Record<Filters["store"], string> = {
  ios: "App Store",
  android: "Google Play",
};
...
const CHART_OPTIONS_BY_STORE: Record<Filters["store"], Filters["chart"][]> = {
  ios: ["free", "paid", "grossing", "new"],
  android: ["free", "paid", "grossing", "new"],
};

function chartLabel(chart: Filters["chart"], store: Filters["store"]): string {
  if (chart === "new" && store === "android") return "New Apps (discovered)";
...
export function ExplorerFilters({ filters, onChange }: Props) {
  ...
  const deviceDisabled = isAndroid || filters.chart === "new";
  ...
  function selectStore(store: Filters["store"]) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 px-6 py-4">
      <Dropdown
        label={<span className="inline-flex items-center gap-1.5"><StoreIcon store={filters.store} className="size-3.5" />{STORE_LABEL[filters.store]}</span>}
        active={filters.store !== DEFAULT_FILTERS.store}
      >
        <div className="flex flex-col gap-0.5">
          {(Object.keys(STORE_LABEL) as Filters["store"][]).map((s) => (
            <DropdownOption
              key={s}
              label={<span className="inline-flex items-center gap-1.5"><StoreIcon store={s} className="size-3.5" />{STORE_LABEL[s]}</span>}
      ...
      <div className={deviceDisabled ? "opacity-40 pointer-events-none" : ""} title={deviceDisabled ? (isAndroid ? "Google Play charts aren't split by device" : "New Apps is a single universal chart — device doesn't apply") : undefined}>

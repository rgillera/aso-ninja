...
export function AnalyzeAllButton({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} className="text-xs text-indigo-400 light:text-indigo-600 hover:text-indigo-300 light:hover:text-indigo-700 transition-colors whitespace-nowrap">
      + Analyze all
    </button>
  );
}
...
export function Toggle({ checked, onChange }: { checked: boolean; onChange: () => void }) {
  return (
    <button
      ...
      onClick={onChange}
      className={`w-8 h-4 min-w-[2rem] min-h-[1rem] shrink-0 grow-0 basis-auto p-0 border-0 appearance-none inline-block rounded-full overflow-hidden transition-colors relative ${checked ? "bg-indigo-500" : "bg-white/10 light:bg-black/[0.12]"}`}
    >
      <span
...
export function VolumeBar({ value, max = 100 }: { value: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-sm text-gray-300 light:text-gray-700 shrink-0">{value}</span>
      <div className="w-16 h-1.5 rounded-full bg-white/[0.08] light:bg-black/[0.08] overflow-hidden">

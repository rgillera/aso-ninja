export function Toggle({ checked, onChange }: { checked: boolean; onChange: () => void }) {
  return (
    <button
      ...
      onClick={onChange}
      className={`w-8 h-4 rounded-full transition-colors relative shrink-0 ${checked ? "bg-indigo-500" : "bg-white/10"}`}
    >
      <span
        className={`absolute top-0.5 size-3 rounded-full bg-white shadow transition-transform ${checked ? "translate-x-4" : "translate-x-0.5"}`}
      />
    </button>
  );
}

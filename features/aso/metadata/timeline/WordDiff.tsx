export function WordDiff({ before, after }: { before: string; after: string }) {
  ...
  return (
    <span className="leading-relaxed">
      {prefix.length > 0 && <span className="text-gray-400 light:text-gray-600">{prefix.join(" ")}</span>}
      ...
      {suffix.length > 0 && <span className="text-gray-400 light:text-gray-600">{" " + suffix.join(" ")}</span>}
    </span>
  );
}

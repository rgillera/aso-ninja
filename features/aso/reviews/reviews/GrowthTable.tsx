...
function GrowthCell({ value, suffix = "" }: { value: number; suffix?: string }) {
  ...
  const up = value > 0;
  return (
    <span className={`inline-flex items-center gap-1 text-sm font-medium ${up ? "text-emerald-400 light:text-emerald-700" : "text-red-400 light:text-red-600"}`}>
    ...
    <div className="rounded-xl bg-[#1a1d24] light:bg-white overflow-hidden">
      <table className="w-full">
        <thead>
          <tr className="border-b border-white/[0.07] light:border-black/[0.08]">
            ...
            <th className="px-5 py-2.5 text-left text-xs font-medium text-gray-500">
              Reviews gained
              <span className="block font-normal text-gray-600 light:text-gray-400 normal-case">{from} – {to}</span>
            </th>
        ...
        <tbody className="divide-y divide-white/[0.04] light:divide-black/[0.06]">
          {rows.map((row) => (
            <tr key={row.label}>
              <td className="px-5 py-2.5 text-sm text-gray-300 light:text-gray-700">{row.label}</td>
              <td className="px-5 py-2.5 text-sm tabular-nums font-medium text-white light:text-gray-900">{row.gained.toLocaleString()}</td>

...
export function NavigationDrawer({
  ...
  return (
    <>
      <button onClick={openDrawer} aria-label="Menu" className="-m-2 p-2 text-gray-400 hover:text-gray-200">
      ...
      <div
        className={`fixed left-0 top-0 z-50 flex h-full w-80 max-w-[85vw] flex-col bg-[#15171d] shadow-2xl transition-transform duration-200 ${open ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-5">
          <h2 className="text-base font-medium text-gray-100">Menu</h2>
          <button onClick={() => setOpen(false)} aria-label="Close" className="-m-2 p-2 text-gray-500 hover:text-gray-200">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <p className="px-5 pb-1.5 pt-5 text-[10px] font-semibold uppercase tracking-widest text-gray-600">
            Workspaces
          </p>
          <ul>
            ...
            {workspaces.map((ws) => (
              <li key={ws.id}>
                <Link
                  href={`/mobile/${ws.id}`}
                  className={`block px-5 py-3.5 text-sm ${ws.id === workspaceId ? "font-medium text-white" : "text-gray-400 hover:text-gray-200"}`}
                >
                  {ws.name}
                </Link>
              </li>
                ...
                <li key={key}>
                  {multiCountry ? (
                    <button
                      ...
                      className={`flex w-full items-center gap-1.5 px-5 py-3.5 text-left text-sm ${activeInGroup ? "font-medium text-white" : "text-gray-400 hover:text-gray-200"}`}
                    >
                    ...
                    <Link
                      ...
                      onClick={() => setOpen(false)}
                      className={`flex items-center gap-1.5 px-5 py-3.5 text-sm ${primary.id === appId ? "font-medium text-white" : "text-gray-400 hover:text-gray-200"}`}
                    >
                      <span className="min-w-0 flex-1 truncate">{primary.name}</span>
                      ...
                      {entries.map((entry) => (
                        <li key={entry.id}>
                          <Link
                            ...
                            onClick={() => setOpen(false)}
                            className={`flex items-center gap-2 py-3 pl-10 pr-5 text-sm ${entry.id === appId ? "font-medium text-white" : "text-gray-400 hover:text-gray-200"}`}
                          >

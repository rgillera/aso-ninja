...
const agentLinks: { label: string; href: string; icon: typeof UserGroupIcon; adminOnly?: boolean }[] = [
  { label: "Contacts", href: "/agents", icon: UserGroupIcon },
  { label: "Summary", href: "/agents/summary", icon: ChartBarIcon, adminOnly: true },
  { label: "Settings", href: "/agents/settings", icon: Cog6ToothIcon },
];

export function AgentsShell({ children, canManage }: { children: React.ReactNode; canManage: boolean }) {
  const pathname = usePathname();
  const [isMobileOpen, setIsMobileOpen] = useState(false);
  const visibleLinks = agentLinks.filter((link) => !link.adminOnly || canManage);
    ...
    <div className="flex h-screen bg-[#111318] light:bg-[#f5f6f8] overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2.5 p-4 border-b border-white/[0.07] light:border-black/[0.08]">
          ...
          <span className="text-sm font-semibold text-white light:text-gray-900">Agent CRM</span>
          <button
            type="button"
            onClick={() => setIsMobileOpen(false)}
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-1">
          {visibleLinks.map((link) => {
            // "/agents" is the index route, so it must match exactly —
            // otherwise it'd also light up as a prefix of "/agents/summary".
            const isActive = link.href === "/agents" ? pathname === link.href : pathname.startsWith(link.href);
            return (
              <a
                key={link.href}
                href={link.href}

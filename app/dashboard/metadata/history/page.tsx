...
export default async function Page() {
  ...
  if (allApps.length === 1) redirect(`/dashboard/apps/${allApps[0].id}/history`);

  return (
    <div className="flex h-screen bg-[#111318] overflow-hidden">
      <DashboardSidebar currentPath="/dashboard/metadata/history" workspaces={allWorkspaces} activeWorkspaceId={firstWorkspace?.id} />
      <main className="flex-1 flex items-center justify-center">
        {allApps.length === 0 ? (
          ...
          <AppPicker apps={allApps} basePath="history" />
        )}
      </main>
    </div>
  );

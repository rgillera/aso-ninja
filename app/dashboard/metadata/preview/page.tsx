...
export default async function Page() {
  ...
  if (allApps.length === 1) redirect(`/dashboard/apps/${allApps[0].id}/preview`);

  return (
    <div className="flex h-screen bg-gray-900 overflow-hidden">
      <DashboardSidebar currentPath="/dashboard/metadata/preview" workspaces={allWorkspaces} activeWorkspaceId={firstWorkspace?.id} />
      <main className="flex-1 flex items-center justify-center">
        {allApps.length === 0 ? (
          ...
          <AppPicker apps={allApps} basePath="preview" />
        )}
      </main>
    </div>
  );
